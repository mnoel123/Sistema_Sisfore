...
@section('title', 'Ver Roles asignados')
...
<div class="container">
  <main class="mt-3">
    ...
    <a href="{{ route('Roles.create') }}" class="btn btn-primary mb-3">Asignar Rol</a>
    ...
    <table id="mitabla" class="table table-striped">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{ $roles['nombre_usuario'] }}</td>
          <td>{{ $roles['email_usuario'] }}</td>
           
          <td>
            <form action="{{ route('Roles.destroy', $roles['id_rol']) }}" method="POST" style="display: inline-block;">
              @csrf
              @method('DELETE')
